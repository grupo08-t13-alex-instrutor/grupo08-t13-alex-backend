...
const updateAdsService = async (
    data: IAdUpdateRequest,
    ...
    const findAd = await advertisementRespository
        .findOneOrFail({
            where: {
                id: advertisementId,
            },
    ...
    const validatedDataResponse = await adsResponseSerializer.validate(
        updatedAd,
        { stripUnknown: true }
    );
