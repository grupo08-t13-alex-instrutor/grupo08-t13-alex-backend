...
export const listAdressService = async (
    ...
    const adress = await adressRepository
        ...
        .catch((reason) => {
            throw new AppError(404, "Address not exist!");
        });

    const validatedDataResponse = await adressResponseSerializer.validate(
        adress,
        { stripUnknown: true }
    );
